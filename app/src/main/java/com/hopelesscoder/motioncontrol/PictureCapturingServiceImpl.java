...
public class PictureCapturingServiceImpl extends APictureCapturingService {
     ...
     */
    @Override
    public void startCapturing(final PictureCapturingListener listener, boolean backCamera) {
        this.picturesTaken = new TreeMap<>();
        this.capturingListener = listener;
        this.cameraIds = new LinkedList<>();
        try {
            ...
            if (cameraIds.length > 0) {
                if (backCamera){
                    for(String cameraId :cameraIds){
                        if(manager.getCameraCharacteristics(cameraId).get(CameraCharacteristics.LENS_FACING)
